...
> ⚠️ **CẬP NHẬT 2026:**
> - Tiếp tục nhắc lại: Hãy dùng `createRoot` thay cho `ReactDOM.render`.
...
> ⚠️ **Mẹo nhỏ:** Bạn có thể truyền bất kỳ thứ gì qua props, kể cả cả một component khác (thường dùng props `children`).
```
...
> ⚠️ **CẬP NHẬT 2026:**
>
> - **PropTypes** hiện nay ít được dùng hơn. Thay vào đó, hầu hết các dự án thực tế dùng **TypeScript** để kiểm tra kiểu dữ liệu (từ lúc viết code).
> - Với **defaultProps**, cách hiện đại hơn cho Function Components là dùng **Default Parameters** của ES6 ngay trong phần khai báo hàm:
>   `const Header = ({ title = 'Default Title' }) => ...`
> - Cách này gọn và tường minh hơn `Header.defaultProps`.
